...
uint8_t i2c_io(uint8_t device_addr, uint8_t *add_ptr, uint16_t addr_n, 
               uint8_t *write_ptr, uint16_t write_n, uint8_t *read_ptr, uint16_t read_n)
{
    ...
    if (addr_n > 0 || write_n > 0) {
        ...
        // Write "addr_n" data bytes to the slave device
        while (addr_n-- > 0) {
            TWDR = *add_ptr++;                   // Put next data byte in TWDR
            TWCR = (1 << TWINT) | (1 << TWEN); // Start transmission
            while (!(TWCR & (1 << TWINT))); // Wait for TWINT to be set
            status = TWSR & 0xf8;
        ...
        // Write "write_n" data bytes to the slave device
        while (write_n-- > 0) {
            TWDR = *write_ptr++;                   // Put next data byte in TWDR
            TWCR = (1 << TWINT) | (1 << TWEN); // Start transmission
            while (!(TWCR & (1 << TWINT))); // Wait for TWINT to be set
            status = TWSR & 0xf8;
            if (status != 0x28) {           // Check that data was sent OK
